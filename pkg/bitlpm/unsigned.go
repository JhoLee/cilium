...
func NewUintTrie[K Unsigned, T any]() Trie[K, T] {
	...
	return &trieUint[K, T]{NewTrie[K, T](size * 8), size}
}
...
func (tu *trieUint[K, T]) getKey(k K) Key[K] {
	...
	// this cannot happen, but the
	// compiler needs it
	return nil
}

func (tu *trieUint[K, T]) Update(prefix uint, k K, value T) {
	tu.t.Update(prefix, tu.getKey(k), value)
}
...
func (tu *trieUint[K, T]) Lookup(prefix uint, k K) T {
	return tu.t.Lookup(prefix, tu.getKey(k))
}
...
func (tu *trieUint[K, T]) ForEach(f ForEachFunc[K, T]) {
	tu.t.ForEach(func(prefix uint, k Key[K], v T) bool {
		return f(prefix, k.Value(), v)
	})
}

type unsignedKey8[U Unsigned] uint8

func (u unsignedKey8[U]) Overlap(v U) uint {
...
func (u unsignedKey16[U]) Overlap(v U) uint {
...
func (u unsignedKey32[U]) Overlap(v U) uint {
...
func (u unsignedKey64[U]) Overlap(v U) uint {
